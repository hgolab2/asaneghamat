...
if ( ! function_exists( 'liquid_woocommerce_get_template' ) ) {
	function liquid_woocommerce_get_template() {
		...
		elseif ('digital' === $style) {
			wc_get_template_part('content', 'product-digital');
			return;
		}
	}
}
...
function liquid_product_size_guide_template( $post_id, $type ) {
	...
	if ( $type == 'modal' ) {
		?>
			<div id="<?php echo esc_attr( 'lqd-modal-product-size-guide' ); ?>" class="lqd-modal lqd-lity-hide" data-modal-type="default">
			...
			<a class="elementor-button btn lqd-size-guide-button mb-4" href="#lqd-modal-product-size-guide" data-lqd-lity="#lqd-modal-product-size-guide">
				<?php
