...
function liquid_woocommerce_get_products_list_view_type()
{
	...
	return liquid_helper()->get_option('wc-archive-grid-list-default');
}
...
if (!function_exists('liquid_woocommerce_product_styles')) {
	function liquid_woocommerce_product_styles($style = '')
	{
		...
		if ('classic' == $style || 'classic-alt' == $style) {
			...
			add_action('woocommerce_shop_loop_item_title', 'woocommerce_template_single_excerpt', 99);
			add_action('woocommerce_shop_loop_item_title', 'woocommerce_template_loop_rating', 99);
