 ...
 */
 function liquid_header_mobile_section() {
	...
	if ( class_exists( 'Liquid_Elementor_Addons' ) && defined('ELEMENTOR_VERSION') ){
		...
		$img_from = liquid_helper()->get_post_meta( 'm-nav-logo-from', $id );

		$img_array = liquid_helper()->get_option( 'menu-logo' );
		$retina_array = liquid_helper()->get_option( 'menu-logo-retina' );

		if( isset( $img_from ) && $img_from === 'custom' ) {
			$img_array = liquid_helper()->get_post_meta( 'm-nav-logo', $id );
			$retina_array = liquid_helper()->get_post_meta( 'm-nav-logo-retina', $id );
		}
		...
		echo '<div class="carousel-container carousel-nav-floated carousel-nav-center carousel-nav-middle carousel-nav-xl carousel-nav-solid carousel-nav-rectangle">';
			...
			echo '<div class="carousel-items row mx-0" data-lqd-flickity=\'{ "prevNextButtons": true, "navArrow": "1", "pageDots": false, "navOffsets":{"prev":"28px","next":"28px"}, "parallax": true, "adaptiveHeight": true }\'>';
				...
				foreach ( $gallery as $item ) {
					...
					printf( '<div class="carousel-item col-xs-12 px-0"><figure class="pos-rel"><img src="%s" alt="%s">%s</figure></div>',$resized_image , esc_attr( get_the_title($item) ), $lightbox === 'on' ? liquid_get_lightbox_link( $resized_image, 'lqd-post-gallery' ) : '' );
...
function liquid_render_related_posts( $post_type = 'post' ) {
	...
	if ( class_exists( 'Liquid_Elementor_Addons' ) && defined('ELEMENTOR_VERSION') ){
		...
		if ( !$heading || !liquid_helper()->is_page_elementor() ){ $heading = liquid_helper()->get_option( $option_name . 'title', 'html' ); }
