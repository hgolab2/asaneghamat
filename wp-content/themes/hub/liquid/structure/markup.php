...
function liquid_attributes_content( $attributes ) {
	...
	if ( (class_exists( 'Liquid_Elementor_Addons' ) && defined('ELEMENTOR_VERSION')) && ( $post_types === 'post' || $post_types === 'page' || $post_types === 'liquid-portfolio' ) ){
		...
		$stack_scroll_speed = $page_settings_model->get_settings( 'page_stack_scroll_speed' );
	} else {
	...
	if( 'on' === $enabled_stack ) {
		...
		if ( !empty( $stack_scroll_speed['size'] ) ) {
			$stack_opts['scrollingSpeed'] = $stack_scroll_speed['size'] . 's';
		}
