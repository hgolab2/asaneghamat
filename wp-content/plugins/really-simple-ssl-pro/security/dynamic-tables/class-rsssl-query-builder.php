...
require_once rsssl_pro_path . 'security/dynamic-tables/class-rsssl-abstract-database-manager.php';
...
class Rsssl_Query_Builder extends Rsssl_Abstract_Database_Manager {
	...
	private function validate_column( string $column ): string {
		...
		if ( ! in_array( $column, $this->raw_columns, true ) && ! in_array( $column, $this->get_columns(), true ) ) {
			throw new RuntimeException( sprintf( "Invalid column %s in select for table %s.", esc_html( $column ),
				esc_html( $this->table ) ) );
		}
		return $column;
	}
