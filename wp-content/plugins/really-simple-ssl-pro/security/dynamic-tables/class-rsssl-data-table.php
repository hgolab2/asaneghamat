...
namespace RSSSL_PRO\Security\DynamicTables;
...
class Rsssl_Data_Table {
	 ...
	 * @param  Rsssl_Query_Builder $query_builder // the query builder instance.
	 */
	public function __construct( array $post, Rsssl_Query_Builder $query_builder ) {
		$this->post          = $post;
	...
	public function set_select_columns( array $array_data ): Rsssl_Data_Table {
		...
		foreach ( $array_data as $key => $column ) {
			...
			if ( false === strpos( $column, 'raw:' ) ) {
				if ( ! in_array( $column, $this->get_columns(), true ) ) {
					$message = sprintf(
						'Invalid column %s in select for table %s.',
						esc_html( $column ),
						esc_html( $this->query_builder->get_table() )
					);
		...
		foreach ( $raw_columns as $raw_column ) {
			...
			if ( isset( $exploded[1] ) ) {
				...
				throw new RuntimeException( esc_html( 'Unexpected rawColumn format: ' . $raw_column ) );
			}
		}
