...
use DateTimeZone;
use Exception;
use RSSSL_PRO\Security\WordPress\Limitlogin\Rsssl_Geo_Location;
...
if ( ! class_exists( 'Rsssl_Event_Log' ) ) {
	...
	class Rsssl_Event_Log {
		...
		public function __construct() {
			require_once rsssl_pro_path . 'security/wordpress/limitlogin/class-rsssl-geo-location.php';
		...
		public static function create_event_log_table(): void {
			...
			if ( get_option( 'rsssl_event_log_db_version' ) !== rsssl_pro_version ) {
				// Create the table.
				global $wpdb;
				...
				dbDelta( $sql );
				update_option( 'rsssl_event_log_db_version', rsssl_pro_version, false );
			}
		}
		...
		public static function log_event( array $data ): void {
			...
			if ( false === $result ) {
				throw new RuntimeException( 'Failed to insert event into the database' );
			}
		}
...
if ( rsssl_get_option( 'enable_limited_login_attempts' ) ) {
	if ( ! function_exists( 'RSSSL_PRO\Security\WordPress\rsssl_event_log_api' ) ) {
		/**
		...
		add_filter( 'rsssl_do_action', 'RSSSL_PRO\Security\wordpress\rsssl_event_log_api', 10, 3 );
	}
}

// Add the cleanup function to the hook.
add_action(
	'rsssl_pro_every_day_hook',
	array( 'RSSSL_PRO\Security\wordpress\rsssl_event_log', 'clean_up_event_log' )
);
