...
class rsssl_pro_admin {
	...
	function __construct() {
		...
		if ( get_option('rsssl_run_activation' ) ) {
	...
	public function activate() {
		...
		update_option('rsssl_run_activation', false, false);
	}
	 ...
	 * @return void
	 *
	 * Set Two Factor defaults
	 */
	public function maybe_set_two_fa_defaults() {
		// Default 2FA usermeta fields
		if ( ! get_option( 'rsssl_default_two_fa_usermeta_set' ) ) {
			// Get all users
			$users = get_users();
			...
			if ( $users ) {
				...
				foreach ( $users as $user ) {
					// Set the default 2FA authentication method to 'open'
					update_user_meta( $user->ID, 'rsssl_two_fa_status', 'open' );
				}
				update_option( 'rsssl_default_two_fa_usermeta_set', true );
			}
		}

		// Set default 2FA optional / forced roles
		if ( ! get_option('rsssl_default_two_fa_roles_set') ) {
			$forced_roles = ['administrator'];
			$optional_roles = ['editor', 'author', 'contributor'];

			rsssl_update_option('two_fa_forced_roles', $forced_roles);
			rsssl_update_option('two_fa_optional_roles', $optional_roles);

			update_option('rsssl_default_two_fa_roles_set', true);
		}
	}
	 ...
	 * @return void
	 *
	 * Show notice when Really Simple SSL free is deactivated
	 */
	public function notice_free_deactivated() {

		$msg = esc_html( __( "You have installed Really Simple SSL Pro. We have deactivated and removed the free plugin.", 'really-simple-ssl' ) );
	...
	public function add_pro_system_status($output){
		$output .= "Really Simple SSL Pro version: " . rsssl_version . "\n";
	...
	public function get_notices_list($notices)
	{
		...
		$notices['redirect_loop_warning'] = array(
			...
			'output' => array(
				...
				'true' => array(
					'title' => __("Potential redirect loop.", "really-simple-ssl"),
					'msg' => __("A redirect to http was detected. This might result in redirect loops.", "really-simple-ssl").' '.sprintf(__("%sRead more%s.", "really-simple-ssl"), '<a target="_blank" href="https://really-simple-ssl.com/knowledge-base/my-website-is-in-a-redirect-loop/">', '</a>')
		...
		$notices['free_compatibility'] = array(
			...
			'output' => array(
				'true' => array(
					'msg' =>  __("Update Really Simple SSL Free to the latest version for optimal compatibility.", "really-simple-ssl"),
					'icon' => 'open',
					'dismissible' => 'true',
					'plusone' => true,
				),
