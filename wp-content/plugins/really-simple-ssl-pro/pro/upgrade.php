...
function rsssl_pro_upgrade( $prev_version ) {
	...
	if ( $prev_version && version_compare( $prev_version, '5.4.0', '<' ) ) {
		$permissions_policy = RSSSL()->admin->get_networkwide_option('rsssl_permissions_policy');
		...
		RSSSL()->admin->update_networkwide_option('rsssl_permissions_policy', $permissions_policy );
		RSSSL()->admin->delete_networkwide_option( 'rsssl_expect_ct' );
		RSSSL()->headers->insert_security_headers( true );
	}

	if ( $prev_version && version_compare( $prev_version, '5.5.1', '<' ) ) {
		if ( RSSSL()->admin->get_networkwide_option('rsssl_security_headers_method') === 'php' ) {
			RSSSL()->admin->update_networkwide_option( 'rsssl_security_headers_method', 'advancedheaders' );
			RSSSL()->headers->insert_security_headers( true );
		}
	}

	if ( $prev_version && version_compare( $prev_version, '5.5.4', '<' ) ) {
		if ( RSSSL()->admin->get_networkwide_option('rsssl_security_headers_method') === 'advancedheaders' ) {
			RSSSL()->headers->insert_security_headers( true );
		}
		if ( RSSSL()->admin->get_networkwide_option('rsssl_security_headers_method') === 'htaccess' ) {
			RSSSL()->admin->update_networkwide_option( 'rsssl_security_headers_method', 'advancedheaders' );
			RSSSL()->headers->insert_security_headers( true );
		}
	}
	...
	if ( $prev_version && version_compare( $prev_version, '6.0.0', '<' ) ) {
		...
		RSSSL()->admin->remove_htaccess_rules('Really_Simple_SSL_SECURITY_HEADERS', true );
		delete_transient( 'rsssl_show_nginxconf_notice' );
		RSSSL()->admin->remove_htaccess_rules( 'Really_Simple_SSL_CSP_Report_Only' );
		RSSSL()->admin->remove_htaccess_rules( 'Really_Simple_SSL_Content_Security_Policy' );
		...
		$new_options['enable_permissions_policy'] = RSSSL()->admin->get_networkwide_option('rsssl_turn_on_permissions_policy');
		$new_options['x_content_type_options'] = RSSSL()->admin->get_networkwide_option('rsssl_x_content_type_options');
		if ( RSSSL()->admin->get_networkwide_option('rsssl_no_referrer_when_downgrade') ) {
			$new_options['referrer_policy'] = 'strict-origin-when-cross-origin';
		} else {
			$new_options['referrer_policy'] = 'disabled';
		}
		$new_options['x_xss_protection'] = RSSSL()->admin->get_networkwide_option('rsssl_x_xss_protection');
		$new_options['admin_mixed_content_fixer'] = RSSSL()->admin->get_networkwide_option('rsssl_admin_mixed_content_fixer');
		$new_options['upgrade_insecure_requests'] = RSSSL()->admin->get_networkwide_option('rsssl_upgrade_insecure_requests');
		$new_options['license'] = RSSSL()->admin->get_networkwide_option('rsssl_pro_license_key');

		if ( RSSSL()->admin->get_networkwide_option('rsssl_x_frame_options') ) {
			$new_options['x_frame_options'] = 'SAMEORIGIN';
		} else {
			$new_options['x_frame_options'] = 'disabled';
		}
		if ( RSSSL()->admin->get_networkwide_option('rsssl_content_security_policy') === 'enforce' ) {
			$new_options['csp_status'] = 'enforce';
		} else if ( RSSSL()->admin->get_networkwide_option('rsssl_content_security_policy') === 'report-only' ) {
			$new_options['csp_status'] = 'learning_mode';
		} else {
			$new_options['csp_status'] = 'disabled';
		}
		$block_third_party_popups = RSSSL()->admin->get_networkwide_option('rsssl_block_third_party_popups');
		$share_resources_third_parties = RSSSL()->admin->get_networkwide_option('rsssl_share_resources_third_parties');
		RSSSL()->admin->delete_networkwide_option('rsssl_block_third_party_popups');
		RSSSL()->admin->delete_networkwide_option('rsssl_share_resources_third_parties');
		// Cors headers
		...
		$hsts = RSSSL()->admin->get_networkwide_option('rsssl_hsts');
		$hsts_preload = RSSSL()->admin->get_networkwide_option('rsssl_hsts_preload');
	...
	if ( $prev_version && version_compare( $prev_version, '8.0.0', '<' ) ) {
		...
		if ( rsssl_get_option( 'enable_limited_login_attempts' ) && ! Rsssl_Login_Attempt::check_if_table_exists() ) {
