...
class rsssl_headers {
	 ...
	 * - activation of Really Simple SSL pro
	...
	public function mark_headers_with_non_recommended_values($fields){
		...
		foreach ($non_recommended_headers as $header => $value ) {
			...
			$fields[$index]['tooltip'] = sprintf(__("The %s security header is not set by Really Simple SSL, but has a non-recommended value: \"%s\".", 'really-simple-ssl'), $header, $value );
		}
		return $fields;
	}
			...
			$notices[ 'wrong_value_'.$header ] = array(
				...
				'output' => array(
					'true' => array(
						...
						'msg' => sprintf(__("The %s security header is not set by Really Simple SSL, but has a non-recommended value: \"%s\".", 'really-simple-ssl'), $header, $value ),
						'icon' => 'open',
						'dismissible' => true,
		...
		if ( count($missing_headers)>0 ) {
			...
			$notices[ 'missing_headers' ] = array(
				...
				'output' => array(
					'true' => array(
						'url' => "https://really-simple-ssl.com/how-to-find-where-security-headers-are-set/",
						'msg' => sprintf(__("The following recommended security headers have not been set: %s", 'really-simple-ssl'), $missing ),
						'icon' => 'open',
						'dismissible' => true,
						'clear_cache_id' => 'detected_headers',
					),
