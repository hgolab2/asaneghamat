...
if ( !defined('GEOIP_DETECT_VERSION') ) require rsssl_path . 'pro/assets/vendor/autoload.php';
...
class Rsssl_Geo_Location {
	...
	private function create_country_table() {
		...
		if ( get_option( 'rsssl_country_db_version' ) !== rsssl_version ) {
			// Create the table.
			global $wpdb;
			...
			update_option( 'rsssl_country_db_version', rsssl_version, false );
		}
	}
	...
	public static function import_countries_from_csv(): void {
		...
		if ( get_option( 'rsssl_country_import_version' ) !== rsssl_version ) {
			...
			$csv_file_path = rsssl_path . 'pro/security/wordpress/limitlogin/countries.txt'; // Replace with the actual path to your CSV file.
			...
			update_option( 'rsssl_country_import_version', rsssl_version, false );
		}
	}
