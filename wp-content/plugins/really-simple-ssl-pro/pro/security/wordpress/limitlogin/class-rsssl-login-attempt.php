...
use REALLY_SIMPLE_SSL\Security\wordpress\Rsssl_Limit_Login_Attempts;
...
class Rsssl_Login_Attempt {
	...
	/**
	 * We create the login attempts table.
	 *
	 * @return void
	 */
	public static function create_login_attempts_table($force = false): void {
		// if the table exists we do not need to create it.
		// only load on front-end if it's a cron job.
		if ( ! rsssl_admin_logged_in()) {
			return;
		}

		if ( get_option( 'rsssl_login_attempts_db_version' ) !== rsssl_version  || $force ) {
			// Create the table.
			global $wpdb;
			...
			dbDelta( $sql );
			update_option( 'rsssl_login_attempts_db_version', rsssl_version, false );
		}
	}
	...
	private function exists_login_attempt( $attempt_type ) {
		...
		$sql = $wpdb->prepare(
			'SELECT COUNT(*) FROM  ' . $wpdb->base_prefix . 'rsssl_login_attempts WHERE attempt_type = %s AND attempt_value IN ( %s, %s )',
			$attempt_type,
			$ip,
			$username
		);
	...
	private function delete_failed_login_attempt(): void {
		...
		$sql = $wpdb->prepare(
			"DELETE FROM {$wpdb->base_prefix}rsssl_login_attempts WHERE attempt_value IN ( %s, %s ) AND ( status IS NULL OR status = %d )",
			$ip,
			$username,
			self::LOCKED
		);
	...
	private function get_login_attempts( string $attempt_type = null ): array {
		...
		if ( null !== $attempt_type ) {
			$sql = $wpdb->prepare(
				"SELECT * FROM {$wpdb->base_prefix}rsssl_login_attempts WHERE attempt_type = %s AND attempt_value IN ( %s, %s )",
				$attempt_type,
				$ip,
				$username
			);
	 ...
	 * @return void
	 */
	public static function cleanup_locked_accounts() {
		global $wpdb;
		$self     = new self( '', '' );
		$table    = $wpdb->base_prefix . self::TABLE;
		...
		$sql = "DELETE FROM {$wpdb->base_prefix}rsssl_login_attempts WHERE (status IS NULL OR status = %s) AND last_failed <= %d";
		// phpcs:ignore WordPress.DB
	...
	/**
	 * This function locks out a user instantly
	 *
	 * @param  string $value
	 * @param  string $type
...
add_action(
	'rsssl_pro_five_minutes_hook',
	array( 'REALLY_SIMPLE_SSL\Security\WordPress\Limitlogin\Rsssl_Login_Attempt', 'cleanup_locked_accounts' )
);
