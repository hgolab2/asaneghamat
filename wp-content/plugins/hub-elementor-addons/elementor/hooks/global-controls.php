...
class LD_Global_Controls extends Module {
	...
	public function __construct() {
		...
		add_action( 'elementor/element/kit/section_typography/after_section_end', array( $this, 'register_typography_section' ), 20, 2 );
		...
		// TODO: fix slowed queries when WooCommerce activated
	...
	public function register_sizeguide_options ( Controls_Stack $element, $section_id ) {
		...
		$element->add_control(
			...
			[
				...
				'multiple' => true,
				'label_block' => true,
				'options' => $this->get_available_products( 'product' ),
				'condition' => [
	...
	public function register_typography_section( Controls_Stack $element, $section_id ) {

		$element->start_controls_section(
			'lqd_typo_section',
			[
				'label' => __( 'Liquid Typography', 'hub-elementor-addons' ),
	...
	public function tweak_background_section( $element ) {
		...
		$element->add_group_control(
			...
			[
				...
				'selector' => 'div#wrap, main#lqd-site-content',
			]
		);
	...
	private function get_custom_taxonomies( $type ) {
	  ...
	  private function get_available_products( $args = [] ) {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return [];
		}
